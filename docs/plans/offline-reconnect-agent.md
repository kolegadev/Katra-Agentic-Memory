# Plan: Offline Reconnect Agent for thebrick

**Author:** Satori · **Date:** 2026-09-05 · **Status:** RESEARCH + PLAN (not yet built)
**Requested by:** John — when thebrick loses internet, fall back to the local LLM already loaded on the box for exactly one task: get the machine reconnected to the router/WiFi and back to LLM inference. Use the `/loop` skill to keep retrying until success. Katra's vault already holds the router SSID/passwords. Investigate Flipper Zero / ESP32 Marauder-style WiFi capabilities to help find and connect to the local network.

---

## 1. Verified machine facts (checked on thebrick today)

| Fact | Value | Implication |
|---|---|---|
| Host | `thebrick`, workspace `/home/johnpellew` | agent runs here, no SSH hop |
| WiFi NIC | `wlp3s0` (`phy0`), **supports monitor mode**, radio not blocked (`rfkill`: soft no / hard no) | host NIC can do passive scans; while offline there is no live connection to disrupt |
| Net stack | `NetworkManager` + `wpa_supplicant` + `systemd-networkd` all active; default route via `192.168.1.1` on `wlp3s0` | `nmcli` is the control plane; saved NM connection profile already contains the PSK (root-only) |
...
| Katra vault | Holds `91D2F8 home router` and `netis home router` secrets (two routers = two candidate networks) | credentials available offline (Katra is local Docker) |
...
- **Linux-native equivalents**: `iw dev wlp3s0 scan` (managed-mode passive scan, no disruption), `airodump-ng`/`wifite2` (monitor mode), `bettercap` `wifi.recon` (can run its WiFi module on a *separate* interface while the main NIC stays managed — relevant if we add a $10 USB monitor-mode dongle).
...
- **Happy path needs no vault:** the NM saved connection profile on thebrick already holds the WiFi PSK (root-only). `nmcli connection up` reuses it — the vast majority of outages are fixed without revealing any secret.
- **Vault is the fallback source of truth** when the profile is lost/corrupt, or to log into the router admin UI (192.168.1.1) to check WAN status:
...
`rfkill unblock wifi`, `nmcli connection up|down|modify|reload`, `nmcli device disconnect|connect|wifi rescan|wifi connect <ssid> password <psk>`, `systemctl restart wpa_supplicant|NetworkManager` (net units only — hardcoded list, not user input), `dhclient -r && dhclient`, `ip link set wlp3s0 up|down`, `iw dev wlp3s0 set type monitor|managed`, `nmcli connection add` (re-create profile from vault), router-admin `curl` (login + WAN status page only), `reboot` as last resort after ≥6 failed cycles (configurable, default off).
...
| DHCP lease lost | no v4 addr on wlp3s0 | `dhclient` renew / `nmcli up` |
...
| Both routers listed | vault has 2 secrets | try `91D2F8` then `netis` (order configurable) |
