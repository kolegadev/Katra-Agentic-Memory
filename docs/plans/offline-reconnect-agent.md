...
| WiFi NIC | the WiFi interface, **supports monitor mode (a generic capability note) (`rfkill`: soft no / hard no) | host NIC can do passive scans; while offline there is no live connection to disrupt |
...
- **Linux-native equivalents**: `iw dev the WiFi interface scan` (managed-mode passive scan, no disruption), `airodump-ng`/`wifite2` (monitor mode), `bettercap` `wifi.recon` (can run its WiFi module on a *separate* interface while the main NIC stays managed — relevant if we add a $10 USB monitor-mode dongle).
...
`rfkill unblock wifi`, `nmcli connection up|down|modify|reload`, `nmcli device disconnect|connect|wifi rescan|wifi connect <ssid> password <psk>`, `systemctl restart wpa_supplicant|NetworkManager` (net units only — hardcoded list, not user input), `dhclient -r && dhclient`, `ip link set the WiFi interface up|down`, `iw dev the WiFi interface set type monitor|managed`, `nmcli connection add` (re-create profile from vault), router-admin `curl` (login + WAN status page only), `reboot` as last resort after ≥6 failed cycles (configurable, default off).
