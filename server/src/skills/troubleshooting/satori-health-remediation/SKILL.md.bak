...
3. **Memory integrity unhealthy** → `get_background_status`: if the processor is alive (last-processed timestamp current, model ready), a **small** backlog (a few thousand events) drains via the normal 12-per-cycle throttle — do not "fix" a draining backlog, verify the count is dropping between checks. A **large** backlog (10k+ events) needs the bulk drain: use the `bulk-backlog-drain` skill (`scripts/drain-backlog.mjs` inside the container). A backlog that is NOT dropping despite a live processor usually means stale `processing` idempotency entries are blocking the queue head — the server self-heals these since the 2026-08-13 fix (stale entries cleared and retried); verify by checking `processing_log` for `processing` entries older than 30 min. If the processor is dead, `docker logs katra-server --tail 100` and restart the container from the repo: `docker compose up -d server`. Stale semantic facts → `trigger_reflection`.
