...
# OpenCode desktop, BEFORE answering anything about identity, memory,
# or what "we've" done.
#
...
#   KATRA_WAKE_KEY  Zanshin's client key (optional if the key file exists).
#   Key file:       ~/.katra/keys/katra-zanshin.key (chmod 600), read
#                   automatically; whitespace in the file is stripped.
#
# Fail-closed: if the identity check cannot confirm "Zanshin" after three
...
if [ "$IDENTITY_NAME" != "$EXPECTED_NAME" ]; then
  ...
  echo "      2. Key file must exist: ~/.katra/keys/katra-zanshin.key" >&2
  echo "         create: printf '%s' '<zanshin-key>' > ~/.katra/keys/katra-zanshin.key" >&2
  echo "         present: $([ -f "$HOME/.katra/keys/katra-zanshin.key" ] && echo yes || echo NO)" >&2
  echo "      3. Re-run: bash ~/.kolega/wake-zanshin.sh" >&2
  exit 1
fi
printf '%s\n' "$IDENTITY_TEXT"
...
mcp_call "search_memories" '{"query": "\"Attention: Zanshin\" OR \"Attention: Katra\" OR \"Attention: Shoshin\" OR \"Attention: Lilly\" OR \"Attention: Zefir\"", "limit": 5}'
...
cat <<'EOF'
  ...
  my-team channel: Katra, Shoshin, Zanshin, Lilly, Zefir.
