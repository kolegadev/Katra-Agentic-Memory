# Shoshin Wake Ritual — survives /clear and /compress
...
and at the start of every session — run the Shoshin wake ritual.

1. Run:  `bash ~/.kolega/wake-shoshin.sh`
   → identity record (must say **name: Shoshin** — the script refuses to wake
...
2. Env (set once in `~/.zshrc` or rely on the key file):
   ...
   Key file: `~/.katra/keys/katra-shoshin.key` (chmod 600) — the script reads
...
4. Bridge health after every `kolega-code update`:
   ...
   with `KATRA_USER_ID=shoshin` — the command hooks are update-proof; this
