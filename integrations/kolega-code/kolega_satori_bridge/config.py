...
def config_path() -> Path:
    ...
    return _default_state_dir() / "satori-hook.json"
...
def _resolve_secret(name: str, file_value: str) -> str:
    ...
    environment) instead of being hardcoded in satori-hook.json. An explicit,
...
def load_config(path: Path | str | None = None) -> BridgeConfig:
    ...
    local = BridgeConfig(
        ...
        # Keeps the key out of satori-hook.json while staying backward-compatible.
