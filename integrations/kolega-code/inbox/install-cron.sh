...
# Zefir on the Katra host. Version-controlled dispatch config — see README.md.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
SCRIPT="$REPO/integrations/kolega-code/scripts/satori_inbox.py"
# Deployment-specific agent policies live in the git-ignored private/ folder.
SATORI_DIR="$REPO/private/integrations/kolega-code/inbox-agent"
ZEFIR_DIR="$REPO/private/integrations/kolega-code/inbox-agent-zefir"
...
crontab -l 2>/dev/null | grep -v "satori_inbox.py dispatch" > /tmp/cron.base || true
{
  cat /tmp/cron.base
  cat <<CRON
# satori inbox auto-reply loop — full capability (operator-approved 2026-09-09)
*/3 * * * * /usr/bin/python3 $SCRIPT dispatch >> $LOG_DIR/cron.log 2>&1
# zefir inbox auto-reply loop — full capability (operator-approved 2026-09-09)
