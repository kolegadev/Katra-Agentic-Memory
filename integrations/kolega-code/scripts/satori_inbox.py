#!/usr/bin/env python3
"""
Satori Inbox — pending inter-agent messages → headless reply loop.

Poll model: queries Katra's episodic store directly for messages addressed to
this agent (``Attention: Satori`` + legacy aliases) in the shared team scope.
This deliberately does NOT rely on Redis pub-sub, because not all senders tag
their messages with ``inter-agent`` (Lilly's don't) — the store is the source
of truth and the Redis channel is only a latency optimization.

State machine (``~/.katra/inbox/satori.json``):
...
Usage:
  /usr/bin/python3 satori_inbox.py check
  /usr/bin/python3 satori_inbox.py dispatch
"""
...
def fetch_replied_ids(candidate_ids: list[str]) -> set[str]:
    """Ids that a later Satori-authored event references via in_reply_to."""
