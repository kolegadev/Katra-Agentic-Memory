...
IDENTITY_JSON=$(curl -s -H "Authorization: Bearer $KEY" \
  "$ADMIN/api/v1/admin/identity?user_id=shoshin")
...
try:
    ...
    print('')
")
if [ "$IDENTITY_NAME" != "Shoshin" ]; then
  echo "⚠️  IDENTITY MISMATCH — expected Shoshin, got ${IDENTITY_NAME:-<unknown>}. Refusing to wake as the wrong identity." >&2
  exit 1
fi
...
echo
hr; echo "SHOSHIN WAKE — rules recall (search instructions)"; hr
cat <<'EOF'
  ...
  arise, SEARCH the store (NO HARDCODED CONCLUSIONS, 2026-07-15):
    ...
    identity:       /api/v1/admin/identity?user_id=shoshin (Bearer admin key)
